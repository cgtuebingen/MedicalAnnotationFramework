...
class SlideLoader(QGraphicsObject):
    ...
    def __init__(self, filepath: str = None):
        ...
        self.slide_handler: SlideHandler = SlideHandler()
        ...
        if filepath is not None:
            ...
            if self.scene():
                if self.scene().views():
                    self.num_lvl: int = self.slide_handler.get_num_lvl()             # total number of _slide level
                    self.slide_lvl_active: int = self.slide_handler.get_num_lvl()    # number of current displayed _slide level
                    self.slide_lvl_goal: int = self.slide_handler.get_num_lvl()      # number of wanted _slide level after zooming
                    self._set_image()
    ...
    def paint(self, p: QPainter, o: QStyleOptionGraphicsItem, widget=None):
        ...
        if not self.in_scene and self.scene().views() is not None:
            self.in_scene = True
            if self.slide_handler.current_file is not None:
                self.refactor_image()
                self._set_image()
    ...
    def load_new_image(self, filepath: str):
        ...
        self.slide_handler.set_slide(filepath)
        if self.in_scene:
            self.refactor_image()
            self._set_image()
    ...
    def refactor_image(self):
        ...
        if self.scene().views():
            ...
            self.slide_handler.update_size(width=self.view_width, height=self.view_height)
            self.num_lvl = self.slide_handler.get_num_lvl()
            self.slide_lvl_active = self.num_lvl
            self.slide_lvl_goal = self.num_lvl
            self.scene_pos = np.array([0, 0])
            self.mouse_pos = self.slide_handler.get_mouse_pos()
    ...
    def _set_image(self):
        ...
        slides = self.slide_handler.get_zoom_stack()
    ...
    def update_image_check(self):
        ...
        if self.scene():  # don't run code without a scene, prevents crashes
            ...
            for lvl in range(min(self.slide_lvl_goal, self.slide_lvl_active),
                ...
                scene_low_right_goal = scene_up_left_goal + self.slide_handler.get_slide_size(lvl) * 2 ** lvl
